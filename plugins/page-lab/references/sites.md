...
| **SITE-B** | Same operator and near-identical markup to SITE-A. Cards at `/video.<id>/<slug>` — **one character apart**, which is enough to make a gate written for one score zero on the other. `body` carries 80px inline padding. Its theme is **JS scheme-detection persisted on first visit**: zero `prefers-color-scheme` CSS blocks, but a fresh profile reads dark under a dark OS and white under a light one, and the first load writes the detected choice to storage — pinning every later reading. |
